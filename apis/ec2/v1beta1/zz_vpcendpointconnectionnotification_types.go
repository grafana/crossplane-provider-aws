...
type VPCEndpointConnectionNotification struct {
	...
	// +kubebuilder:validation:XValidation:rule="!('*' in self.managementPolicies || 'Create' in self.managementPolicies || 'Update' in self.managementPolicies) || has(self.forProvider.connectionNotificationArn) || has(self.initProvider.connectionNotificationArn)",message="connectionNotificationArn is a required parameter"
